...
RUN apt-get install -y libfmt-dev=6.1.2+ds-2
...
RUN apt-get update -y
RUN apt-get install -y gcc-11=11.4.0-2ubuntu1~20.04 g++-11=11.4.0-2ubuntu1~20.04
# Set gcc-11 as the default gcc version and g++-11 as the default g++ version.
RUN update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-11 100
RUN update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-11 100

RUN apt-get install python3=3.8.2-0ubuntu2 -y 
RUN apt-get install python3-pip=20.0.2-5ubuntu1.9 -y 
