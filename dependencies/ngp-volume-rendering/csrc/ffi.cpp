...
pybind11::bytes make_packbits_descriptor(std::uint32_t const n_bytes)
{
    ...
    return to_pybind11_bytes(PackbitsDescriptor{ .n_bytes = n_bytes });
}
...
pybind11::bytes make_morton3d_descriptor(std::uint32_t const length)
{
...
PYBIND11_MODULE(cuda_ffi, m) 
{
    ...
    m.def(
        "make_morton3d_descriptor", &make_morton3d_descriptor,
