...
integrate_rays_p.def_impl(partial(xla.apply_primitive, integrate_rays_p))
integrate_rays_p.def_abstract_eval(abstract.integrate_rays_abstract)

integrate_rays_bwd_p = jax.core.Primitive("integrate_rays_backward")
integrate_rays_bwd_p.multiple_results = True
integrate_rays_bwd_p.def_impl(partial(xla.apply_primitive, integrate_rays_bwd_p))
integrate_rays_bwd_p.def_abstract_eval(abstract.integrate_rays_backward_abstract)

integrate_rays_inference_p = jax.core.Primitive("integrate_rays_inference")
integrate_rays_inference_p.multiple_results = True
integrate_rays_inference_p.def_impl(partial(xla.apply_primitive, integrate_rays_inference_p))
...
def __integrate_rays(
    near_distance: float,
    rays_sample_start_idx: jax.Array,
    ...
    bgs = jax.numpy.broadcast_to(bgs, (rays_sample_start_idx.shape[0], 3))
    measured_batch_size, final_rgbds, final_opacities = integrate_rays_p.bind(
        rays_sample_start_idx, rays_n_samples, bgs, dss, z_vals, drgbs,
    )
...
def __fwd_integrate_rays(
    ...
    aux = {
        ...
        "in.rays_sample_start_idx": rays_sample_start_idx,
...
def __bwd_integrate_rays(aux, grads):
    # dL_dfinal_rgbds should be zeros everywhere.
    _, dL_dfinal_rgbds, dL_dfinal_opacities = grads
    dL_dbgs, dL_dz_vals, dL_ddrgbs = integrate_rays_bwd_p.bind(
        aux["in.rays_sample_start_idx"],
    ...
    return (
        ...
        None,
        # The next 2 primal inputs are integer-valued arrays, 
        # (rays_sample_startidx, rays_n_samples), return no gradient for them.
        # REF:
