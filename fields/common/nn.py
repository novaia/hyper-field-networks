...
class MultiResolutionHashEncoding(nn.Module):
    ...
    def __call__(self, x:jnp.ndarray):
        ...
        #point_offset = jnp.transpose(jnp.reshape(scaled - scaled_f, (self.spatial_dim, self.num_levels)))
        ...
        hashed_4 = hash_fn(vertex_4, self.table_size, self.hash_offset)
        hashed_5 = hash_fn(vertex_5, self.table_size, self.hash_offset)
        hashed_6 = hash_fn(vertex_6, self.table_size, self.hash_offset)
        hashed_7 = hash_fn(vertex_7, self.table_size, self.hash_offset)
...
class MultiResolutionHashEncoding2D(nn.Module):
    ...
    def hash_function(self, x:jnp.ndarray, table_size:int, hash_offset:jnp.ndarray):
        x = jnp.repeat(jnp.expand_dims(x, axis=-1), repeats=2, axis=-1)
        pre_xor = x * jnp.array([1, 2654435761])
        x = jnp.bitwise_xor(pre_xor[..., 0:1], pre_xor[..., 1:2])
        x = x % table_size
        x = x + hash_offset
        return x
