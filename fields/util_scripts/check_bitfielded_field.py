...
# python -m fields.util_scripts.check_bitfielded_field --field data/image_field.npy --config configs/ngp_image_robot_benchmark.json --image_width 1920 --image_height 1920
...
def main():
    ...
    # For some reason the initial render gets saved as all black unless it gets saved
    # before the recovered render is created.
    save_render(path='data/bitfield_initial_field_render.png', rendered_image=initial_params_render)
    ...
    recovered_params = unflatten_params(
        flat_params=fpt.bitfield16_to_fp32(fpt.fp32_to_bitfield16(flattened_params)), 
        param_map=param_map
    )
    state = state.replace(params=recovered_params)
    recovered_params_render = render_fn(state=state) 
    
    mse = jnp.mean((recovered_params_render - initial_params_render)**2)
    print('MSE between initial_params_render and detokenized_params_render:', mse)

    save_render(path='data/bitfield_recovered_field_render.png', rendered_image=recovered_params_render)
