...
def train_step(
    discriminator_state, generator_state, real_data, latent_width, latent_height, key
):
    latent_noise = jax.random.normal(key, (real_data.shape[0], latent_height, latent_width))
...
def main():
    latent_height = 8
    latent_width = 8
    generator = Generator(widths=[32, 32, 32], block_depth=2)
    discriminator = Discriminator(widths=[32, 32, 32], block_depth=2)

    learning_rate = 3e-4
    optimizer = optax.adam(learning_rate)

    key = jax.random.PRNGKey(0)
    latent_input = jnp.ones((1, latent_height, latent_width, 3))
