...
# python -m hypernets.util_scripts.byte_pair_tokenize_field_dataset --dataset data/colored-primitives-ngp-image-2291 --out data/colored-primitives-ngp-image-2291-byte-pair
...
def save_table(table_data, context_length, table_number, output_path, zfill_amount):
    ...
    schema = pa.schema(
        fields=[
            ('tokens', pa.list_(pa.uint8(), list_size=context_length)),
        ],
        metadata={
            b'huggingface': json.dumps({
                'info': {
                    'features': {
                        'tokens': {'_type': 'Sequence', 'feature': {'_type': 'Value', 'dtype': 'uint8'}},
                    }
                }
            }).encode('utf-8')
        }
...
def main():
    ...
    context_length_out = context_length * 2
    ...
    for i in range(num_samples):
        tokens = fpt.byte_pair_tokenize(dataset[i]['params'])
        pq_row_data = {
        ...
        if samples_in_current_table >= samples_per_table:
            save_table(
                pq_table_data, context_length_out, current_table_index, out_data_path, 4
            )
            pq_table_data = []
            samples_in_current_table = 0
