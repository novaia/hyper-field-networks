...
def convert_split_field_conv_ae(model_config_dict: dict, args: argparse.Namespace):
    ...
    encoder_save_path = os.path.join(
        abs_output_path, 
        f'{model_config.model_name}_encoder.safetensors'
    )
    decoder_save_path = os.path.join(
        abs_output_path,
        f'{model_config.model_name}_decoder.safetensors'
    )
    save_file(flatten_dict(cpu_params['encoder']), encoder_save_path)
    print(f'Saved encoder to: {encoder_save_path}')
    save_file(flatten_dict(cpu_params['decoder']), decoder_save_path)
...
def main():
    ...
    parser.add_argument(
        '--output', type=str, default='data/safetensors/',
        help='Directory where the safetensors file(s) will be saved'
    )
